...
const HeartIcon = ({ filled }) => (
  <svg className={`w-4 h-4 md:w-5 md:h-5 ${filled ? 'fill-red-500 stroke-red-500' : 'stroke-slate-400 fill-none'} transition-colors`} viewBox="0 0 24 24" strokeWidth="1.8">
    <path strokeLinecap="round" strokeLinejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />
  </svg>
);

const ShareIcon = () => (
  <svg className="w-4 h-4 md:w-5 md:h-5 stroke-slate-400 fill-none hover:stroke-slate-600 transition-colors" viewBox="0 0 24 24" strokeWidth="1.8">
...
const TransmissionIcon = () => (
  <svg className="w-3.5 h-3.5 md:w-4 md:h-4 stroke-slate-400 fill-none" viewBox="0 0 24 24" strokeWidth="1.8">
...
const CheckBadgeIcon = () => (
  <svg className="w-3 h-3 md:w-3.5 md:h-3.5 text-emerald-500 fill-none stroke-current" viewBox="0 0 24 24" strokeWidth="2">
    <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
  </svg>
);

const TagIcon = () => (
  <svg className="w-3.5 h-3.5 md:w-4 md:h-4 text-blue-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
...
const carBrands = [
  ...
  { name: 'Dayun', logo: 'https://upload.wikimedia.org/wikipedia/commons/2/22/Dongfeng_Motor_logo.svg' },
...
export default function VehicleSalesSection() {
  ...
  return (
    <section className="relative py-10 md:py-16 px-4 md:px-8 font-['Montserrat',sans-serif] overflow-hidden min-h-screen">
      ...
      <div className="max-w-7xl mx-auto relative z-10 w-full">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 md:mb-10 gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 md:gap-2 px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-blue-300 text-[10px] md:text-xs font-bold uppercase tracking-wider mb-2 md:mb-3 backdrop-blur-md">
              <TagIcon />
            ...
            <p className="text-xs md:text-base text-slate-300 mt-1.5">
        ...
        <div className="mb-6 md:mb-10 bg-slate-900/60 backdrop-blur-md p-4 md:p-6 rounded-2xl md:rounded-3xl shadow-xl border border-white/10">
          <div className="flex items-center justify-between mb-4">
            <span className="text-[10px] md:text-xs font-bold text-slate-300 uppercase tracking-wider">
              Filtrer par marque ({carBrands.length} disponibles)
            </span>
            {selectedBrand && (
              <button
            ...
            {carBrands.map((brand) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-2 md:p-3 rounded-xl md:rounded-2xl transition-all duration-300 w-16 md:w-28 ${
                    isSelected
                ...
                >
                  ...
                  <span className="text-[9px] md:text-xs font-bold tracking-tight text-center truncate w-full">
                    {brand.name}
                  </span>
                </button>
              );
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 md:gap-4 mb-6 md:mb-8 pb-4 border-b border-white/10">
          <div className="flex items-center gap-2 overflow-x-auto scrollbar-none">
            <button
              onClick={() => setFilter('ALL')}
              className={`px-3 py-1.5 md:px-5 md:py-2.5 text-[10px] md:text-sm font-semibold rounded-full transition-all whitespace-nowrap backdrop-blur-md ${
                filter === 'ALL'
          ...
          <div className="inline-flex items-center gap-1.5 md:gap-2 bg-white/10 backdrop-blur-md px-3 py-1.5 md:px-4 md:py-2 rounded-full border border-white/15 text-[10px] md:text-xs font-bold text-white shadow-sm">
            <span className="w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-emerald-500 animate-pulse" />
            <span>{filteredVehicles.length} véhicule(s)</span>
          </div>
        </div>
              ...
              >
                ...
                <div className="px-4 pb-4 md:px-5 md:pb-5 pt-1 md:pt-2 flex flex-col gap-2.5 md:gap-3">
                  ...
                  <button 
                    ...
                    className="w-full py-2 md:py-2.5 bg-slate-900 hover:bg-blue-600 text-white font-bold text-[11px] md:text-xs rounded-lg md:rounded-xl shadow-sm hover:shadow-md transition-all duration-200 flex items-center justify-center gap-2"
                  >
